...
describe('Settings screen, Test a card behaviors', () => {
    ...
    it('click on Edit button of first card, show the Edit Screen', () => {
        ...
        cy.get('[data-test="scan-card"]').first().find('[data-test="card-edit"]').click();
    ...
    it('click on Copy button of first card, show the Copy Screen', () => {
        ...
        cy.get('[data-test="scan-card"]').first().find('[data-test="card-copy"]').click();
        cy.get('[data-test="settingItem-component"]').should('be.visible')
    ...
    it('click on Delete button of first card, cards are deleted', () => {
        cy.get('[data-test="scan-card"]').should('have.length', 3)
        cy.get('[data-test="scan-card"]').first().find('[data-test="card-delete"]').click();
        cy.get('[data-test="scan-card"]').should('have.length', 2)
        cy.get('[data-test="scan-card"]').first().find('[data-test="card-delete"]').click();
        cy.get('[data-test="scan-card"]').should('have.length', 1)
        cy.get('[data-test="scan-card"]').first().find('[data-test="card-delete"]').click();
        cy.get('[data-test="scan-card"]').should('have.length', 0)
    })
});
...
describe('Settings screen, Test Speed Dial', () => {
    ...
    it("given a messy data, click on Reset Button, will reset default-clean data", () => {
        ...
        cy.get('[data-test="scan-card"]').first().find('[data-test="card-delete"]').click();
        cy.get('[data-test="scan-card"]').first().find('[data-test="card-delete"]').click();
        cy.get('[data-test="scan-card"]').should('have.length.lt', 3)
